...
//    g_.setZero();
    ...
    // For non linear
    H_ = 0.01*H1 + H2 + 0.1*H3;
    g_ = 0.01*g1 + g2 + 0.1*g3;
...
//   ubA_.resize(rcols+pcols+1);
  ...
  lbA_.resize(rcols+pcols+2);
  ubA_.resize(rcols+pcols+2);
   lbA_.setZero();
  ...
  A_.block(rcols + pcols+1,0,1,5) << obsPartielVertices.block(0,0,3,1).transpose(),0,0 ;
